...
- 可尽力注入埋点事件桥；iframe 内接收知乎 HTTPS 父窗口的 postMessage 明文 JSON，独立打开时自动上报默认 PageShow。前端构建统一从允许的 npm `build:track` script 进入；完全静态且没有 package 时生成最小根 package。模板、CSP 或 script 冲突只提示 warning，不影响应用部署。
