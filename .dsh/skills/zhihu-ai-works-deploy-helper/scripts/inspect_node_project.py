#!/usr/bin/env python3
"""Inspect a project, apply optional deterministic source preparation, and emit a TCB deploy plan."""
...
from backend_html_pages import load_html_pages

from event_tracking_bridge import (
    TRACKING_BUILD_SCRIPT,
    TRACKING_PACKAGE_MARKER,
    TRACKING_PACKAGE_MARKER_VERSION,
    TICKET_MATERIALIZER_PATH,
    event_tracking_bridge_contract,
)

from cloudbase_runtime import CLOUD_FUNCTION_RUNTIMES, cloud_function_runtime

from skill_config import MANAGED_BUILD_ENVIRONMENT_NAMES, MANAGED_RUNTIME_ENVIRONMENT
...
MANAGED_BUILD_ENV_NAMES = frozenset(MANAGED_BUILD_ENVIRONMENT_NAMES)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--backend-html-pages", type=Path, help="Agent-reviewed HTML paths and backend route source evidence JSON")
...
def is_generated_tracking_package(path: Path) -> bool:
    ...
    try:
        ...
        return False
    return value == {
        "name": "aiworks-event-tracking-build",
        "private": True,
        "scripts": {TRACKING_BUILD_SCRIPT: f"node {TICKET_MATERIALIZER_PATH}"},
        TRACKING_PACKAGE_MARKER: TRACKING_PACKAGE_MARKER_VERSION,
    }
...
def discover_packages(root: Path) -> list[Package]:
    ...
    if not root_manifest_path.is_file() or is_generated_tracking_package(root_manifest_path):
        return []
...
def inspect_build_free_static(root: Path, findings: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    generated_package = is_generated_tracking_package(root / "package.json")
    if ((root / "package.json").exists() and not generated_package) or not (root / "index.html").is_file():
        return None
    ...
    for current, dirs, names in os.walk(root, topdown=True, followlinks=False):
        ...
        for name in names:
            path = current_path / name
            relative = relative_to_root(path, root)
            if generated_package and relative == "package.json":
                continue
            lowered = name.lower()
...
def framework_contract(
    ...
    exact_versions = contract.get("ExactVersions", [])
    exact_match = isinstance(version, str) and isinstance(exact_versions, list) and version in exact_versions
    supported = exact_match or (
        isinstance(major, int)
        and isinstance(minimum, int)
        and isinstance(maximum, int)
        and minimum <= major < maximum
    )
    ...
    resolved = {
        ...
        "Match": (
            "agent-build-verified"
            if not supported and validation is not None
            else "exact-version"
            if exact_match
            else "version-range"
        ),
...
def static_backend_build_supported(
    ...
    # A leading shell environment assignment does not change the executable
    # or its arguments, so normalize it before matching the closed resolver
    # command set (for example, VAR=value vinext build).
    while True:
        assignment = re.match(
        ...
        if not assignment:
            break
        normalized = normalized[assignment.end():].lstrip()
    if framework == "nestjs":
...
def inspect_user_environment_accesses(
    ...
    for path, raw_text in sources:
        ...
        for name, offset in matches:
            if name in MANAGED_RUNTIME_ENV_NAMES or name in MANAGED_BUILD_ENV_NAMES:
                continue
...
def main() -> int:
    ...
    frontend_component = next((item for item in components if item.get("Unit") == "frontend"), None)
    if args.backend_html_pages:
        backend_page_component = next((item for item in components if item.get("DeliveryMode") == "node-http"), None)
        if backend_page_component is None or frontend_component is not None:
            raise SystemExit("error: --backend-html-pages requires a standalone node-http component")
        try:
            backend_page_component["HtmlPages"] = load_html_pages(root, args.backend_html_pages)
            for page in backend_page_component["HtmlPages"]:
                for artifact in backend_page_component.get("BuildArtifacts", []):
                    output_path = PurePosixPath(artifact["Path"])
                    target_path = PurePosixPath(page["Path"])
                    if output_path == target_path or output_path in target_path.parents:
                        raise ValueError("backend HTML review must identify source HTML, not a build artifact")
        except (OSError, UnicodeError, ValueError) as error:
            raise SystemExit(f"error: invalid backend HTML review: {error}") from error
    page_component = frontend_component or next(
        (item for item in components if item.get("DeliveryMode") == "ssr-node" or item.get("HtmlPages")),
        None,
    )
    event_tracking_bridge = event_tracking_bridge_contract(
        root,
        page_component,
        allow_injection=not any(item["Severity"] == "error" for item in findings),
    )
    if event_tracking_bridge["Status"] == "skipped":
        add_finding(
            findings,
            "W_EVENT_TRACKING_BRIDGE_SKIPPED",
            "warning",
            "project",
            event_tracking_bridge["Evidence"],
            event_tracking_bridge["Reason"],
            "Provide a supported deterministic frontend template or resolve the recorded source/CSP conflict; bridge injection remains optional and does not block delivery.",
        )
    ...
    plan = {
        ...
        "EventTrackingBridge": event_tracking_bridge,
        "Findings": findings,
    }
    if has_backend:
