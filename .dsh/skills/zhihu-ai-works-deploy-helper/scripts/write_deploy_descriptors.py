...
from event_tracking_bridge import (
    ...
    ticket_materialization_command,
    ticket_materialization_source,
    tracking_frontend_build_command,
    tracking_package_conflict,
    tracking_package_path,
    tracking_package_script,
)
...
def ticket_materializer_source_for_plan(plan: dict[str, Any]) -> str | None:
    ...
    if frontends and frontends[0].get("DeliveryMode") != "static-files":
        build_command = frontends[0].get("BuildCommand")
    return ticket_materialization_source(plan.get("EventTrackingBridge"), build_command)


def tracking_package_for_plan(output_root: Path, plan: dict[str, Any]) -> tuple[Path, dict[str, Any]] | None:
    bridge = plan.get("EventTrackingBridge")
    if not isinstance(bridge, dict) or bridge.get("Status") != "injected":
        return None
    frontends = [
    ...
    if len(frontends) != 1:
        return None
    component = frontends[0]
    conflict, evidence = tracking_package_conflict(output_root, component)
    if conflict:
        suffix = f" ({', '.join(evidence)})" if evidence else ""
        raise ValueError(conflict + suffix)
    path = tracking_package_path(output_root, component)
    if path.exists():
        value = json.loads(path.read_text(encoding="utf-8"))
    else:
        value = {
    ...
    scripts = dict(value.get("scripts", {}))
    scripts[TRACKING_BUILD_SCRIPT] = tracking_package_script(output_root, path)
    value["scripts"] = scripts
    value[TRACKING_PACKAGE_MARKER] = TRACKING_PACKAGE_MARKER_VERSION
    return path, value
...
def frontend_descriptor(
    component: dict[str, Any],
    output_root: Path,
    install_cmd: str,
    event_tracking_bridge: Any = None,
) -> dict[str, Any]:
    ...
    try:
        materialize = ticket_materialization_source(
            event_tracking_bridge, build_command=None if static_files else build,
        )
    except ValueError as error:
        raise SystemExit(f"error: {error}") from error
    if materialize is not None:
        try:
            build_value = tracking_frontend_build_command(output_root, component)
        except ValueError as error:
            raise SystemExit(f"error: {error}") from error
    else:
        build_value = "" if static_files else build
    return {
        "SchemaVersion": DEPLOY_SCHEMA_VERSION,
        "Env": [],
        "Framework": framework,
        "NodeVersion": node_version,
        "BuildCmd": build_value,
...
def backend_descriptor(plan: dict[str, Any], component: dict[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    build_command = build_step(component)
    has_frontend = any(
        isinstance(item, dict) and item.get("Unit") == "frontend"
        for item in plan.get("Components", [])
    )
    if not has_frontend:
        try:
            materialize = ticket_materialization_command(plan.get("EventTrackingBridge"))
        except ValueError as error:
            raise SystemExit(f"error: {error}") from error
        if materialize:
            build_command = f"{materialize} && {build_command}"
    custom_steps = [
        custom_step("安装构建依赖", "npm ci"),
        custom_step("构建或验证项目", build_command),
    ]
    if mode == "overlay":
...
def main() -> int:
    ...
    try:
        materializer_source = ticket_materializer_source_for_plan(plan)
        tracking_package = tracking_package_for_plan(output_root, plan)
    except ValueError as error:
        raise SystemExit(f"error: {error}") from error
    materializer_path = output_root / TICKET_MATERIALIZER_PATH
    if materializer_path.is_symlink():
        raise SystemExit(f"error: refusing to replace symlinked {TICKET_MATERIALIZER_PATH}")
    written: list[str] = []
    if materializer_source is not None:
        write_text_atomic(materializer_path, materializer_source)
        written.append(str(materializer_path))
    elif materializer_path.exists():
        if not materializer_path.is_file():
            raise SystemExit(f"error: refusing to replace non-file {TICKET_MATERIALIZER_PATH}")
        materializer_path.unlink()
    if tracking_package is not None:
        package_path, package_value = tracking_package
        if package_path.is_symlink():
            raise SystemExit(f"error: refusing to replace symlinked {package_path.relative_to(output_root)}")
        write_json_atomic(package_path, package_value)
        written.append(str(package_path))
    if frontends:
        path = descriptor_dir / "frontend.deploy.json"
        write_json_atomic(path, frontend_descriptor(
            frontends[0], output_root, args.frontend_install_cmd, plan.get("EventTrackingBridge"),
        ))
