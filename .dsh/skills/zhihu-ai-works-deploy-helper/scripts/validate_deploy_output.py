...
from backend_html_pages import validate_html_pages
from event_tracking_bridge import TICKET_MATERIALIZER_PATH, validate_event_tracking_bridge
...
from write_deploy_descriptors import (
    backend_descriptor,
    frontend_descriptor,
    ticket_materializer_source_for_plan,
    tracking_package_for_plan,
)
...
def validate_plan_semantics(plan: dict[str, Any], errors: list[str]) -> None:
    errors.extend(validate_event_tracking_bridge(None, plan.get("EventTrackingBridge")))
    bridge = plan.get("EventTrackingBridge")
    components = [item for item in plan.get("Components", []) if isinstance(item, dict)]
    has_frontend = any(item.get("Unit") == "frontend" for item in components)
    has_page = has_frontend or any(item.get("DeliveryMode") == "ssr-node" or item.get("HtmlPages") for item in components)
    for component in components:
        if "HtmlPages" in component:
            try:
                pages = validate_html_pages(None, component["HtmlPages"])
                if component.get("DeliveryMode") != "node-http" or has_frontend:
                    raise ValueError("HtmlPages requires a standalone node-http component")
                if isinstance(bridge, dict) and bridge.get("Status") == "injected":
                    if bridge.get("TargetFiles") != [page["Path"] for page in pages] or bridge.get("Adapter") != "backend-html":
                        raise ValueError("backend HTML pages do not match injected bridge targets")
            except ValueError as error:
                errors.append(f"deploy-plan.json: {error}")
    if isinstance(bridge, dict):
        bridge_status = bridge.get("Status")
        bridge_warnings = [
            item for item in plan.get("Findings", [])
            if isinstance(item, dict) and item.get("Code") == "W_EVENT_TRACKING_BRIDGE_SKIPPED"
        ]
        if bridge_status in {"injected", "skipped"} and not has_page:
            errors.append("deploy-plan.json: EventTrackingBridge applies only to a page-rendering component")
        if bridge_status == "not-applicable" and has_page:
            errors.append("deploy-plan.json: page-rendering plan cannot mark EventTrackingBridge not-applicable")
        if bridge_status == "skipped" and len(bridge_warnings) != 1:
            errors.append("deploy-plan.json: skipped EventTrackingBridge requires one W_EVENT_TRACKING_BRIDGE_SKIPPED finding")
        if bridge_status != "skipped" and bridge_warnings:
            errors.append("deploy-plan.json: W_EVENT_TRACKING_BRIDGE_SKIPPED requires skipped bridge status")
...
def validate_project(root: Path, plan: dict[str, Any], schemas: Path, errors: list[str]) -> None:
    errors.extend(validate_event_tracking_bridge(root, plan.get("EventTrackingBridge")))
    for component in plan.get("Components", []):
        if isinstance(component, dict) and "HtmlPages" in component:
            try:
                validate_html_pages(root, component["HtmlPages"])
            except (OSError, UnicodeError, ValueError) as error:
                errors.append(f"backend HTML source review: {error}")
    try:
        expected_tracking_package = tracking_package_for_plan(root, plan)
    except (OSError, UnicodeError, json.JSONDecodeError, ValueError) as error:
        errors.append(f"cannot reconstruct event tracking npm script: {error}")
        expected_tracking_package = None
    if expected_tracking_package is not None:
        package_path, expected_package = expected_tracking_package
        if not package_path.is_file() or package_path.is_symlink():
            errors.append(f"missing regular event tracking package: {package_path.relative_to(root)}")
        else:
            actual_package = load_json(package_path, errors)
            if actual_package is not None:
                expected_scripts = expected_package.get("scripts", {})
                actual_scripts = actual_package.get("scripts")
                if (
                    not isinstance(actual_scripts, dict)
                    or actual_scripts.get("build:track") != expected_scripts.get("build:track")
                    or actual_package.get("aiworksEventTrackingBuild") != 1
                ):
                    errors.append(
                        f"{package_path.relative_to(root)} event tracking npm script does not match the deterministic contract"
                    )
    materializer_path = root / TICKET_MATERIALIZER_PATH
    try:
        expected_materializer = ticket_materializer_source_for_plan(plan)
    except ValueError as error:
        errors.append(f"cannot reconstruct event tracking ticket materializer: {error}")
        expected_materializer = None
    if expected_materializer is not None:
        if not materializer_path.is_file() or materializer_path.is_symlink():
            errors.append(f"missing regular {TICKET_MATERIALIZER_PATH}")
        else:
            try:
                actual_materializer = materializer_path.read_text(encoding="utf-8")
            except (OSError, UnicodeError) as error:
                errors.append(f"cannot read {TICKET_MATERIALIZER_PATH}: {error}")
            else:
                if actual_materializer != expected_materializer:
                    errors.append(f"{TICKET_MATERIALIZER_PATH} does not match the deterministic contract")
    elif materializer_path.exists() or materializer_path.is_symlink():
        errors.append(f"project without an injected event bridge contains {TICKET_MATERIALIZER_PATH}")
    ...
    if isinstance(steps, list):
        ...
        if len(commands) < 2 or not isinstance(commands[1], str) or not ("test -e " in commands[1] or "test -f " in commands[1]):
