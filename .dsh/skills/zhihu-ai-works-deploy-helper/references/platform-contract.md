...
CloudBase 官方资料用于确认该协议依赖的平台能力是否仍成立。平台冲突门禁只适用于已证实且影响当前目标运行时的具体不兼容；报告必须指出适用的函数类型、运行时版本、明确不支持的行为及直接证据。通用文档的启动示例或“使用绝对路径”的措辞，不能单独证明当前 `RuntimeLaunch` 的 PATH / npm 启动方式不可用。未执行云端部署仅表示兼容性未实测，不构成打包门禁。

`scf_bootstrap` 是本 Skill 生成的产物。首次准备或重复执行普通 Node 项目时，均按当前计划生成并校验它；已确认归属的旧产物过期或命令不同应重新生成，不能反过来视为用户项目的启动冲突。正常准备不要求用户裁决 Skill 的启动实现；仅有疑虑时记录待核验项并继续准备，不能声称已经发生启动失败。

若具体证据已确认官方平台行为与下游协议不兼容：
...
- 有适用于目标函数类型和运行时的具体证据表明官方行为与当前生成命令不一致；
- Agent 准备建议改变 Node 版本、bootstrap、端口、部署参数、认证参数或函数目录；
- 有具体平台变更公告影响当前交付方式；仅缓存日期较早不触发每次准备的强制查证。
...
网络或官方页面不可访问时，不把查证失败本身当成项目不可部署的证据。正常准备沿用当前生成器和协议，完成校验、归档并如实披露未核验项。仅当已有具体失败或变更证据影响交付正确性、且继续交付必须依赖尚未证实的平台变更时，才报告查证未完成并阻断。报告须列出未能访问的官方 URL/帮助命令、原始错误、安全重试方法和查证成功后的复检步骤。不得用第三方资料填补权威证据。
