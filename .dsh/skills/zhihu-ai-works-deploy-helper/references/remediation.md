...
- 先区分用户源码问题与 Skill 交付产物过期。已确认由 Skill 生成的旧 `scf_bootstrap`、描述符及运行时准备文件，按新计划重新生成后校验，不因再次执行或新旧启动命令不同而阻断，不要求用户批准 Skill 自身启动规则。普通 Node 的 `exec npm run start` / `exec node` 由当前 RuntimeLaunch 决定；通用文档措辞或未进行云端实测不能单独升级为平台冲突。
...
`AIWORKS_EVENT_TRACKING_TICKET` 是 HostingService 在构建期注入的变量，不进入本阻断流程。它由 ticket 物化步骤消费，不由运行时 bootstrap 导出。
...
适用：`W_EVENT_TRACKING_BRIDGE_SKIPPED`。

- 这是可选增强的 warning，不是打包门禁。事件桥的 endpoint、ticket 占位符和知乎父页面 origin 策略均为固定协议；应按 finding 证据处理模板定位、CSP 或 marker 冲突后重新探测。
- 无法唯一定位首屏模板、CSP 需要框架专用改写、已有冲突 bridge marker，或前端 package 已有不同的 `build:track`/归属 marker 时，保持源码和用户 npm script 不变并继续交付；不得为了消除 warning 猜测修改编译产物、动态 CSP、业务入口或覆盖用户脚本。
- 若项目后来提供了受支持的确定性模板，从原始隔离源码重新探测。成功状态必须记录 `injected` 并通过脚本内容、SHA-256、endpoint、marker 和目标文件校验。
