...
- Node/SSR 运行时源码只能读取 Skill 明确注入的 `PORT`、`HOST`、`HOSTNAME`、`NITRO_HOST` 和 `NITRO_PORT`。由 HostingService 仅在构建期注入的 `AIWORKS_EVENT_TRACKING_TICKET` 是受管构建变量，环境变量扫描放行它，但 bootstrap 不导出它；读取其他 `process.env` 变量时以 `E_USER_ENVIRONMENT_VARIABLE_UNSUPPORTED` 阻断，不尝试判断变量或值是否敏感。向用户明确说明改写后所有值都会以明文进入项目源码和 ZIP、任何可访问交付物的人都能读取；只有用户明确授权将全部命中值固化为项目内引用后，才在隔离交付源码中做最小改写并从头探测。未授权、无回复或含糊授权均保持阻断。
...
- 静态前端、SSR 和后端源码明确关联 HTML 的 Node 页面项目会尽力注入埋点事件桥，并将 endpoint 固定为 `https://tracking.aiworks.site/api/v1/event_tracking`。iframe 内只接收知乎 HTTPS 父窗口通过 `postMessage` 发送的第一条 JSON object；独立打开时在页面加载完成后自动上报一条默认 PageShow，使用当前 URL、当前毫秒时间戳和 `is_iframe_show: 0`。固定 JS 从 script data attribute 读取 ticket，描述符在正式构建或 build-free 发布前用 `AIWORKS_EVENT_TRACKING_TICKET` 原子替换属性占位符，再由桥强制覆盖 `deploy_uuid`。注入成功时交付确定性的 `_tmp/materialize-event-tracking-ticket.cjs`，并在前端 package 中注入 `scripts.build:track`；完全静态且没有 package 时生成带归属 marker 的根 `package.json`。前端 `BuildCmd` 只能以允许的 `npm run build:track` 入口执行，workspace 与普通子目录分别保留 `--workspace` 与 `--prefix` 定位，不直接执行 `node`、shell 或 `echo`。helper 启动时向流水线输出 `AIWORKS_EVENT_TRACKING_TICKET=present|missing`，只披露变量是否存在，不得输出 ticket 值；物化 ticket 后通过子进程运行原构建命令，且不得改变内联 JS 正文或 SHA-256。已有不同的 `build:track` 或归属 marker 时以 `W_EVENT_TRACKING_BRIDGE_SKIPPED` 跳过，不覆盖用户脚本。其他无法唯一定位源码模板、CSP 需要专用改写或已有冲突 marker 的情况同样跳过并保持不阻断交付。确认仅提供 API、没有页面关联的 HTTP 后端才记录为 `not-applicable`；不能仅凭 `backend-http` 分类跳过页面审查。不得从 CLI、项目源码或运行时消息覆盖 endpoint 和 ticket。
...
只有具体证据确认当前目标运行时不支持下游协议生成的交付方式时，才按平台冲突停止生成并列出双方证据。通用文档的启动示例、绝对路径措辞、未实测云端启动，或再次执行时发现旧的 Skill 产物，都不足以判定冲突。普通 Node 项目按当前 RuntimeLaunch 生成启动文件并完成校验、归档，不要求用户为 Skill 自身的启动规则作选择。不允许 Agent 自动降低、替换或扩展协议，也不得忽略已证实的平台不兼容。仅当问题被证明属于项目事实时，才进入源码自愈。
...
`scf_bootstrap`、部署描述符和运行时准备文件是 Skill 管理的交付产物，不是用户必须满足的启动前置条件。重复执行时，确认旧文件归属后，从实际源码重新探测，并由当前生成器再生成这些文件；旧启动命令与新计划不同、产物已存在或内容过期本身都不构成阻断。只有归属不明的自定义文件需要先确认，不能要求用户为已确认的 Skill 产物再次授权覆盖。
...
对于普通 `node-http` 项目，Agent 必须从后端入口沿本地模块、实际注册的路由和静态托管逻辑检查是否提供 HTML。确认 `sendFile`、静态目录或文件读取结果最终进入 HTTP 响应后，把关联的 HTML 源文件及源码证据写入 `_tmp/backend-html-pages.json`，并以 `--backend-html-pages <json>` 重新探测；多个页面全部列出，具体格式见 [后端 HTML 页面审查](references/detection.md#后端-html-页面审查)。这是 Agent 的源码语义审查，脚本只校验记录并确定性注入，不会自行解析任意 JavaScript 路由。仅发现 HTML 文件、字符串引用、未注册路由、文档或测试示例都不能作为关联依据。不得为了开启埋点而把单体 Node 项目改成前后端分离。无法确认的关联需在交付说明中披露，不声称已覆盖。重新探测时必须继续传入经复核的页面记录。

可传入已审查的 `--frontend-dir`、`--backend-dir`、构建命令、`--frontend-output-dir`、`--backend-output-dir`、`--backend-entry`、Node 版本、路由前缀和健康检查路径。Agent build 消歧还必须传 `--build-validation <json>`；探测器会核对实际命令、入口和产物，不接受只有路径覆盖而没有验证链的结果。事件桥无法注入只产生 warning，不得转成打包门禁。
...
Build-free 静态目录省略 `--frontend-install-cmd`。事件桥未注入时最终只能有：
...
事件桥为 `injected` 时还必须生成 `_tmp/materialize-event-tracking-ticket.cjs`，并注入前端 `scripts.build:track`；完全静态且原本没有 package 时还生成根 `package.json`。这些是前端 `BuildCmd` 使用的确定性交付输入，不是后端输入。
...
- 每个计划都含 `EventTrackingBridge`：静态前端和 SSR 页面为 `injected` 或带 warning 的 `skipped`，纯 HTTP 后端为 `not-applicable`；带前端的 `injected` 必须通过 npm `build:track`、helper 内容、hash、endpoint、ticket 占位符、父页面 origin 策略、marker 与目标源码复检。
